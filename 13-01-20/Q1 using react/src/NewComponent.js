		...
		return (
			<div>
				<div style={{float:"left", display:"block", position:"relative"}}>
					...
					<input type="text" placeholder="enter CITY to match..." onChange={this.matched}/>
					...
					<ul style={{color:"blue"}}>
						{list.map((dt)=>(<li onClick={()=>this.displayFunction(dt)} key={dt.id}>{dt.id},  {dt.city},  {dt.pollutant_id}, Station: {dt.station}</li>))}
					</ul>
				</div>
				<div style={{width:"550px",color:"red", float:"right", display:"block", position:"absolute",marginLeft:"700px"}}>
					...
					<ul>
						{this.state.current_item.map((item)=>(<li key={item.id}>{item.id}  {item.city}  min: {item.pollutant_min}, max: {item.pollutant_max}, </li>))}
					</ul>
				</div>				
			</div>
		)
